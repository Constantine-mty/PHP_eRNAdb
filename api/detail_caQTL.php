...
if (preg_match('/^([a-zA-Z0-9]+):(\d+)-(\d+)$/', $select_pos, $matches)) {
    ...
    $map = [];
    $map[] = ['CHR_SNP', '=', $chromosome];
    $map[] = ['Position', '>', $start_position];
    $map[] = ['Position', '<', $end_position];
...
//$totalRecords 总条目的数量计数
    $totalRecords = Db::table('caQTL')->where($map)->count();


//$totalRecordwithFilter 符合筛选条件的条目的数量统计
    $totalRecordwithFilter = Db::table('caQTL')->where($map)->


//调用闭包函数query并且允许使用外部变量searchQuery
    where(function ($query) use ($searchQuery) {
        $query->whereOr([
            ["SNP", "like", $searchQuery],
            ["CHR_SNP", "like", $searchQuery],
            ["Position", "like", $searchQuery],
        ]);
    })->
    field('SNP, CHR_SNP, Position, Ref_allel, Alt_allel')->count();

//'SNP, CHR_SNP, Position, Ref_allel, Alt_allel, Gene, distanceToTSS, Annotation, Effect_size, Celltype, GWASID, Trait'

    $result = Db::table('caQTL')->where($map)->
...
//调用闭包函数query并且允许使用外部变量searchQuery
    ...
    field('SNP, CHR_SNP, Position, Ref_allele, Alt_allele')->
