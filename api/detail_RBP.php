...
//$totalRecords 总条目的数量计数
    $totalRecords = Db::table('human_rbp_deoverlap')->where($map)->count();


//$totalRecordwithFilter 符合筛选条件的条目的数量统计
    $totalRecordwithFilter = Db::table('human_rbp_deoverlap')->where($map)->
...
//调用闭包函数query并且允许使用外部变量searchQuery
    ...
    field('C1, C2, C3, C4, C5, C6, C7, C8')->count();



    $result = Db::table('human_rbp_deoverlap')->where($map)->
