...
$totalRecords = Db::table('marker_gene')->where([
    ['dataset','=',$sid],
])->count();


//$totalRecordwithFilter 符合筛选条件的条目的数量统计
$totalRecordwithFilter = Db::table('marker_gene')->where([
    ['dataset','=',$sid],
])->
...
where(function ($query) use ($searchQuery) {
    $query->whereOr([
        ["names", "like", $searchQuery],
        ["Cluster", "like", $searchQuery],
    ]);
})->count();
...
$result = Db::table('marker_gene')->where([
    ['dataset','=',$sid],
])
    ->where(function ($query) use ($searchQuery) {
    $query->whereOr([
