...
//$totalRecords 总条目的数量计数
    $totalRecords = Db::table('human_Common_SNP')->where($map)->count();


//$totalRecordwithFilter 符合筛选条件的条目的数量统计
    $totalRecordwithFilter = Db::table('human_Common_SNP')->where($map)->
...
//调用闭包函数query并且允许使用外部变量searchQuery
    ...
    field('C4, C1, C2, C5, C6')->count();



    $result = Db::table('human_Common_SNP')->where($map)->
