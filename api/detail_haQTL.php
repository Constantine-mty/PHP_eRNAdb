...
if (preg_match('/^([a-zA-Z0-9]+):(\d+)-(\d+)$/', $select_pos, $matches)) {
    ...
    $map = [];
    $map[] = ['Chr', '=', $chromosome];
    $map[] = ['Pos', '>', $start_position];
    $map[] = ['Pos', '<', $end_position];
...
//$totalRecords 总条目的数量计数
    $totalRecords = Db::table('human_haQTL')->where($map)->count();


//$totalRecordwithFilter 符合筛选条件的条目的数量统计
    $totalRecordwithFilter = Db::table('human_haQTL')->where($map)->


//调用闭包函数query并且允许使用外部变量searchQuery
    where(function ($query) use ($searchQuery) {
        $query->whereOr([
            ["Chr", "like", $searchQuery],
            ["Pos", "like", $searchQuery],
        ]);
    })->
    field('Chr, Pos, Ref, Alt, Tissue')->count();



    $result = Db::table('human_haQTL')->where($map)->
