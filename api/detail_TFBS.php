...
if (preg_match('/^([a-zA-Z0-9]+):(\d+)-(\d+)$/', $select_pos, $matches)) {
    ...
    $map = [];
    $map[] = ['chrom', '=', $chromosome];
    $map[] = ['start', '>', $start_position];
    $map[] = ['end', '<', $end_position];


//$totalRecords 总条目的数量计数
    $totalRecords = Db::table('human_TFBS')->where($map)->count();


//$totalRecordwithFilter 符合筛选条件的条目的数量统计
    $totalRecordwithFilter = Db::table('human_TFBS')->where($map)->
...
//调用闭包函数query并且允许使用外部变量searchQuery
    ...
    field('chrom, start, end, TF, strand')->count();



    $result = Db::table('human_TFBS')->where($map)->
...
//调用闭包函数query并且允许使用外部变量searchQuery
    ...
    field('chrom, start, end, TF, strand')->
